...
angular
    ...
    .module('app.editarcategoria', ['angularFileUpload'])


    .controller('EditarCategoriaCtrl', ['$scope', 'FileUploader','$http','$ionicModal', '$timeout', '$stateParams','$location', function($scope, FileUploader,$http, $ionicModal, $timeout, $stateParams,$location) {
    ...
    id = $stateParams.id;
      ...
      $scope.editarCategoria = function (categoria){
        console.log($scope.cardapio.categorias[$stateParams.id] != null);
        console.log($stateParams.id);
        $scope.cardapio != $scope.cardapio.categorias.splice($stateParams.id,1,categoria);      
        $http.post("editarcardapio.php", $scope.cardapio).success(function (data) {
          /*delete $scope.cardapio;*/
          console.log("/app/cardapio/"+namespace);
